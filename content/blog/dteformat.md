...
Work compels me to look at a lot of monthly time series charts. I like ggplot, and I use it for most of my work-charting needs, but I always struggle to get a date axis that's comprehensible but compact. 
...
Not terrible. But not great. That's about as compact as it gets while still being comprehensible with the standard % formats, as far as I can tell. 
...
We'll tell scale_x_date to use that instead...
...
It doesn't work so well if you want to label the axis every six months (J12, J12, J13, J13, J14, J14). Every three months (J12, A12, J12, O12) kind of works, but not really. Every other month is maybe OK, but it still gets crowded. 

Realistically, this format probably only works well when your date_breaks are set to "1 month" so the pattern (JFMAMJJASOND) is intuitive. Which means that if you have more than a couple of years of data, this is a poor choice. 
...
Looks good, except now we have no idea what year each point belongs to. That tends to be an important detail. Excel, Tableau, etc. make this easy - the x-axis can be a couple of lines.
...
For my purpose and audience, the best results would either be the rotated "J04, F04, M04" format, or the plot with the annotated years. Right now, I'm leaning towards the latter. This approach seems to meet the criteria of being comprehensible and uncluttered. It also meets the criteria of not taking too much time to generate. As opposed to tinkering with grid layouts, using annotation also seems likely to be easier to understand from the perspective of the poor coworker who might need to read my code. There isn't anything going on but basic ggplot, and a really simple custom format function.
